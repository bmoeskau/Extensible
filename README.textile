...
<p>If you run the build script (e.g., "sh build.sh" in the Mac terminal) it will build everything (minus docs), create a <code>/deploy</code> folder and also place the *-all files in the root folder.  The jar file for the Ext-doc tool ("http://ext-doc.org/":http://ext-doc.org/) is also included.  If you'd like to generate docs also, simply pass "-d" as a command line arg to the build script (e.g., "sh build.sh -d").</p>
